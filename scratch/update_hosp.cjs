...
code = code.replace(
  /export async function nearestHospitals\(\n\s*lat: unknown,\n\s*lng: unknown,\n\s*limit = 3,\n\s*radiusKm = 25,\n\s*\): Promise<\{ hospitals: Record<string, unknown>\[\]; note: string \| null \}> \{/,
  `export async function nearestHospitals(
  lat: unknown,
  lng: unknown,
  limit = 3,
...
code = code.replace(
  /const \{ data, error \} = await admin\(\)\.rpc\(\"hospitals_nearby\", \{\n\s*p_lat: latitude,\n\s*p_lng: longitude,\n\s*p_radius_km: clamp\(radiusKm, 1, 100\),\n\s*p_type: null,\n\s*p_speciality: null,\n\s*p_limit: clamp\(Math\.trunc\(limit\), 1, 20\),\n\s*p_offset: 0,\n\s*\}\);/,
  `const { data, error } = await admin().rpc("hospitals_nearby", {
      p_lat: latitude,
      p_lng: longitude,
...
console.log('Updated server/routes/hospitals.ts!');
